...
func TestArrayIndexExpressionParsing(t *testing.T) {
	tests := []struct {
		input    string
		expected interface{}
	}{
		...
		{
			"let myArr = [1, 2, 3]; let idx = myArr[0]; myArr[idx];",
			2,
		},
		{
			"[1, 2, 3][3]",
...
func TestFunctionApplication(t *testing.T) {
	tests := []struct {
		...
		{"fn(x) { x; }(5);", 5},
		{"let f = fn(x) { return x * 2; }; let g = fn(y) { return (y + 1) * 3; }; f(g(2));", 18},
	}
	for _, tt := range tests {
		testIntegerObject(t, testEval(tt.input), tt.expected)
	}
...
func TestBuiltinFunction(t *testing.T) {
	tests := []struct {
		...
		{`let myArr = [1, 2, 3]; push(myArr, 4);`, []int{1, 2, 3, 4}},
		{`push(1, 1);`, "first argument to `push` must be ARRAY, got INTEGER"},
		{`push(["a"], 1, 3)`, "wrong number of arguments. got=3, want=2"},
	}
